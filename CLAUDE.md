...
Rechnung und Angebot teilen denselben geführten 3-Schritt-Flow (Kunde → Positionen → Vorschau/Versand), unterschieden per `docType: "offer" | "invoice"`. Drafts sind persistent (`documents.status = 'draft'`); leere Drafts werden wiederverwendet bzw. nach 30 min aufgeräumt. Schritt 1 (Kunde) ist überspringbar — Empfängerangaben werden betragsabhängig erst über 250 € brutto Pflicht (§ 33 UStDV).
