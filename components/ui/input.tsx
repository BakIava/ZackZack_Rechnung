...
type InputProps = React.ComponentProps<"input"> & {
  /** Min. 48px height for touch-friendly forms */
  touch?: boolean;
};

function Input({ className, type, touch = false, ...props }: InputProps) {
  return (
    <InputPrimitive
      type={type}
      data-slot="input"
      className={cn(
        ...
        touch ? "min-h-12 px-4 py-3 text-base" : "h-8 px-2.5 py-1 md:text-sm",
        className
      )}
      {...props}
    />
